...
bool check_straight(int hand[5][2])
{
    ...
    if (max - min + 1 == 5)     // 연속숫자 5개라. 1,2,3,4,5... 3,4,5,6,7, 경우..
    {
...
bool check_duplicate(int card_idx, int hand[5][2], int rank, int suit)
{
    ...
    for (i = 0; i < card_idx; i++) {    // card_idx -> 저장한 카드 숫자만큼 비교
        if (rank == hand[i][0])         // 숫자 문양 중복시 is_duplicate = true;
            if (suit == hand[i][1]) {
                is_duplicate = true;
                break;
            }
    }
    return is_duplicate;        // 중복x -> false, 중복 -> true;
}
