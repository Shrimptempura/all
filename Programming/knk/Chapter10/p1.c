...
void push(char ch)
{   
    // push 전에, 스택이 풀인지 확인, 풀일시 stack_overflow();
    if (is_full())
        stack_overflow();
    else {
        printf("%c is pushed.\n", ch);
...
char pop(void)
{
    // pop 전에, 스택이 비었는지 확인, 비었을시 stack_underflow();
    if (is_empty())
        stack_underflow();
    else {
        printf("%c is popped.\n", contents[top-1]);
...
void stack_underflow(void)
{
    ...
    underflow_check = true;     // 외부변수 false에서 true로 바뀜;
}
...
bool enter_and_check_input(void)
{
    ...
    while ((ch = getchar()) != '\n')
    {
        // is_parenthesis_or_brace : 괄호면 true, 아니면 false 반환
        if (is_parenthesis_or_brace(ch) != true)
            continue;
        else    // 괄호일때 switch문으로 push or pop;
            switch (ch) {
                case '{': case '(':
                    push(ch); break;
                case '}': case ')':
